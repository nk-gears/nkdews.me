---
title: About me
slug: me
template: page
---

I’m Nirmal, Web and Software Enthusiast living in Chennai. I am independent Freelancer designing and architecting solutions for Cloud.


####  🔹 Cloud Expertise 
- AWS, Azure & Google Cloud Platform
- Load Balancing, SSL Configuration, Automated Alerts
- Serverless Applications, Event-Driven Architecture
- Kubernetes & Container Orchestration

####  🤖 AI & Chatbot (Including Voice) Specialist  
- LLMs & Generative AI: Vertex AI, Gemini 1.5 Pro, OpenAI, Assembly AI
- ChatBot & VoiceBot Development: Voximplant, Dialogflowcx, Twilio, VAPI Claude Haiku

####  💻 Full-Stack Polyglot Developer
- Languages: PHP, C#, Python, NodeJS, JavaScript
- Containerization: Docker, Swarm, Portainer
- Workflow Automation: Google App Scripts, SharePoint (SPFx)

#### 🔄 Business Workflow Automation - AppScript Based
- Google Apps Script expert
- Custom scripts for Google Workspace (Sheets, Docs, Forms, Drive)
- Integration with external APIs and services
- Automated reporting and data processing
- Time-driven and event-driven triggers


####  🛠️ DevOps & Infrastructure
- CI/CD Pipelines
- Private Container Registries
- Kubernetes in Public/On-Premise Environments
- Airflow on Kubernetes

####  💬 Client Feedback
> "Amazing work! Finished before time & under budget."

> "Developed a high-quality solution with quick understanding."

> "Great developer, prioritizes client needs. Highly recommended!"

Checkout my full testimonials : https://nkdews.me/testimonial/

####  🌱 Always Learning
I thrive in dynamic environments, embracing new technologies and challenges. My goal is to deliver efficient, scalable solutions tailored to your unique needs.

**Let's build something amazing together! 🤝**


####  Currently Using

> 2024 

- **Computer:** MacBook Pro 2015
- **Operating System:** Primary : Mac OSX Catalina. (Windows,Linux on demand)
- **Hosting:** [Netlify](https://netlify.com)
- **Editor:** [Visual Studio Code](https://code.visualstudio.com/)
- **Static Site Generator:** [Hugo]
- **Quick Notes:** [SimpleNote](https://simplenote.com/)
